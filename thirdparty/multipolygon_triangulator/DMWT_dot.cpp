...
void DMWT::statistics(){
	timeTotal = timePreprocess+timeMWT+timeDelaunay;
	if(!DO_EXP){
		...
		cout<<" (T) Call Delaunay:\t"<<timeDelaunay<<endl;
		...
		cout<<numofpoints<<"\t"<<numofedges<<"\t"<<numoftris<<"\t"
			...
			<<timeDelaunay<<"\t"<<timePreprocess<<"\t"<<timeMWT<<"\t"
...
bool DMWT::tileSegment(int eind, char side, int ti, float& thisCost, int& thisTile){
	...
	if(isboard){
		costtribd = 0.0f;
		if(tindofti!=-1){
			if(side==MWT_RIGHT) costtribd = costTriBd(measureTriBd(ev1,ev2,tv3,ev1));
		...
		for(int t=0; t<tnum; t++){
			...
			if(ti==-1){
				if (useWorstDihedral) {
					...
					if(weightTriBd!=0.0f){
						if(side==MWT_LEFT)  costtribd = costTriBd(measureTriBd(ev1,ev2,v3,ev1));
