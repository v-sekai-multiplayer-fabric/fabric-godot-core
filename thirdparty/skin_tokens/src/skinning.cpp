...
result<std::vector<vec3>> deform_vertices(
    ...
    if (geometry.vertices.empty()) {
        return std::unexpected(detail::fail(error_code::invalid_argument, "mesh has no vertices"));
    }
    if (joint_count == 0U || binding.rig.rest_positions.size() != joint_count ||
        binding.rig.names.size() != joint_count) {
        return std::unexpected(detail::fail(error_code::invalid_argument, "skin skeleton arrays do not agree"));
    }
    ...
    if (animation.frames == 0U || frame >= animation.frames) {
        return std::unexpected(detail::fail(error_code::invalid_argument, "animation frame is out of range"));
    }
    if (!compatible(binding.rig, animation.rig)) {
        return std::unexpected(detail::fail(error_code::invalid_argument, "skin and animation skeletons are incompatible"));
    }
    ...
    if (!detail::checked_mul(animation.frames, joint_count, rotation_count) ||
        ...
        return std::unexpected(detail::fail(error_code::invalid_argument, "animation arrays do not agree"));
    }
    ...
    for (std::size_t joint = 0U; joint < joint_count; ++joint) {
        ...
        if (!finite(translation) || !finite(rotation)) {
            return std::unexpected(detail::fail(error_code::invalid_argument, "animation contains non-finite transforms"));
        }
    ...
    for (std::size_t vertex = 0U; vertex < geometry.vertices.size(); ++vertex) {
        ...
        if (!finite(source)) {
            return std::unexpected(detail::fail(error_code::invalid_argument, "mesh contains non-finite vertices"));
        }
        double x = 0.0;
        double y = 0.0;
        double z = 0.0;
        ...
        if (weight_sum <= 0.0) {
            return std::unexpected(detail::fail(error_code::invalid_argument, "skin vertex has no positive influence"));
        }
