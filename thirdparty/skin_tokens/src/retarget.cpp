...
result<humanoid_mapping> match_generated_humanoid(
    ...
    auto valid=validate(generated);if (!valid) return std::unexpected(valid.error());
    ...
    const auto fixed_chain=[&](std::size_t start,std::size_t wanted) -> std::optional<std::vector<std::size_t>> {
    ...
    std::optional<std::vector<std::size_t>> trunk;
    ...
    for (const auto candidate:children[root]) {
        ...
        if (!path.empty() && children[path.back()].size()>=3U) {
            if (trunk) return std::unexpected(detail::fail(error_code::invalid_argument,
    ...
    for (const auto child:children[root]) {
        ...
        auto chain=fixed_chain(child,4U);
        if (!chain) return std::unexpected(detail::fail(error_code::invalid_argument,
...
result<motion> retarget_soma30_motion_to_generated(
    ...
    auto valid_source=validate(animation);if (!valid_source) return std::unexpected(valid_source.error());
    auto valid_target=validate(generated);if (!valid_target) return std::unexpected(valid_target.error());
    if (!std::isfinite(options.minimum_confidence) || options.minimum_confidence<0.0F ||
    ...
    for (std::size_t index=0;index<soma_names.size();++index) {
        ...
        if (!joint) return std::unexpected(detail::fail(error_code::invalid_argument,
    ...
    if (source_height<=1.0e-8F || target_height<=1.0e-8F)
        return std::unexpected(detail::fail(error_code::invalid_argument,"cannot scale root motion for a degenerate humanoid"));
...
result<motion_divergence_report> compare_soma30_to_generated(
    ...
    auto valid_source=validate(source_motion);if (!valid_source) return std::unexpected(valid_source.error());
    auto valid_target=validate(target_motion);if (!valid_target) return std::unexpected(valid_target.error());
    ...
    if (!target_head) return std::unexpected(target_head.error());
    if (!target_left_toe) return std::unexpected(target_left_toe.error());
    if (!target_right_toe) return std::unexpected(target_right_toe.error());
    ...
    if (!target_root_result) return std::unexpected(target_root_result.error());
