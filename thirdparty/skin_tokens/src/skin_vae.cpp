...
std::vector<float> embed_condition(std::span<const float> points, std::span<const float> normals) {
...
std::vector<float> embed_condition(std::span<const vec3> points, std::span<const vec3> normals) {
...
result<tensor_snapshot> run_skin_vae_fixture(const weight_component & weights, ggml_backend_t backend,
    ...
    if (backend == nullptr || points.size() != static_cast<std::size_t>(points_count * 3) || normals.size() != points.size())
        return std::unexpected(fail(error_code::invalid_argument, "SkinVAE fixture must contain 64 point/normal triples"));
    ...
    if (context == nullptr) return std::unexpected(fail(error_code::allocation, "cannot create SkinVAE graph"));
    ...
    if (allocator == nullptr || !ggml_gallocr_reserve(allocator, graph) || !ggml_gallocr_alloc_graph(allocator, graph)) {
        ...
        return std::unexpected(fail(error_code::allocation, "cannot allocate SkinVAE graph"));
    }
    ...
    if (ggml_backend_graph_compute(backend, graph) != GGML_STATUS_SUCCESS)
        return std::unexpected(fail(error_code::compute, "SkinVAE graph execution failed"));
...
result<std::vector<float>> encode_skin_condition(const weight_component & weights, ggml_backend_t backend,
    ...
    if (backend == nullptr || points.empty() || normals.size() != points.size() || query_indices.empty())
        return std::unexpected(fail(error_code::invalid_argument, "invalid SkinVAE condition input"));
    for (const auto index : query_indices)
        if (index < 0 || static_cast<std::size_t>(index) >= points.size())
            return std::unexpected(fail(error_code::invalid_argument, "SkinVAE condition query index is out of range"));
    ...
    if (context == nullptr) return std::unexpected(fail(error_code::allocation, "cannot create SkinVAE condition graph"));
    auto cleanup = std::unique_ptr<ggml_context, decltype(&ggml_free)>(context, ggml_free);
    ...
    if (allocator == nullptr || !ggml_gallocr_reserve(allocator, graph) || !ggml_gallocr_alloc_graph(allocator, graph)) {
        if (allocator != nullptr) ggml_gallocr_free(allocator);
        return std::unexpected(fail(error_code::allocation, "cannot allocate SkinVAE condition graph"));
    }
    auto allocator_cleanup = std::unique_ptr<ggml_gallocr, decltype(&ggml_gallocr_free)>(allocator, ggml_gallocr_free);
    ggml_backend_tensor_set(input, embedding.data(), 0, embedding.size() * sizeof(float));
    ggml_backend_tensor_set(indices, query_indices.data(), 0, query_indices.size_bytes());
    if (ggml_backend_graph_compute(backend, graph) != GGML_STATUS_SUCCESS)
        return std::unexpected(fail(error_code::compute, "SkinVAE condition graph execution failed"));
...
result<std::vector<float>> decode_skin_joint(const weight_component & weights, ggml_backend_t backend,
                                              ...
                                              std::span<const float> condition_latents,
                                              std::span<const vec3> points,
                                              std::span<const vec3> normals) try {
    if (backend == nullptr || codes.size() != 4U || points.empty() || normals.size() != points.size() ||
        ...
        return std::unexpected(fail(error_code::invalid_argument, "invalid SkinVAE decoder input"));
    ...
    for (std::size_t token = 0; token < codes.size(); ++token) {
        ...
        if (codes[token] < 0 || codes[token] > 32768)
            return std::unexpected(fail(error_code::invalid_argument, "SkinVAE code is out of range"));
        for (std::size_t digit = 0; digit < 5U; ++digit)
    ...
    {
        auto * context = ggml_init({256ULL << 20U, nullptr, true});
        if (context == nullptr) return std::unexpected(fail(error_code::allocation, "cannot create SkinVAE cache graph"));
        auto cleanup = std::unique_ptr<ggml_context, decltype(&ggml_free)>(context, ggml_free);
        auto * code_input = ggml_new_tensor_2d(context, GGML_TYPE_F32, 5, skin_tokens);
        ...
        if (allocator == nullptr || !ggml_gallocr_reserve(allocator, graph) || !ggml_gallocr_alloc_graph(allocator, graph)) {
            if (allocator != nullptr) ggml_gallocr_free(allocator);
            return std::unexpected(fail(error_code::allocation, "cannot allocate SkinVAE cache graph"));
        }
        auto allocator_cleanup = std::unique_ptr<ggml_gallocr, decltype(&ggml_gallocr_free)>(allocator, ggml_gallocr_free);
        ...
        if (ggml_backend_graph_compute(backend, graph) != GGML_STATUS_SUCCESS)
            return std::unexpected(fail(error_code::compute, "SkinVAE cache graph execution failed"));
    ...
    for (std::size_t begin = 0; begin < points.size(); begin += vertex_chunk) {
        ...
        auto * context = ggml_init({256ULL << 20U, nullptr, true});
        if (context == nullptr) return std::unexpected(fail(error_code::allocation, "cannot create SkinVAE query graph"));
        auto cleanup = std::unique_ptr<ggml_context, decltype(&ggml_free)>(context, ggml_free);
        ...
        if (allocator == nullptr || !ggml_gallocr_reserve(allocator, graph) || !ggml_gallocr_alloc_graph(allocator, graph)) {
            if (allocator != nullptr) ggml_gallocr_free(allocator);
            return std::unexpected(fail(error_code::allocation, "cannot allocate SkinVAE query graph"));
        }
        auto allocator_cleanup = std::unique_ptr<ggml_gallocr, decltype(&ggml_gallocr_free)>(allocator, ggml_gallocr_free);
        ...
        if (ggml_backend_graph_compute(backend, graph) != GGML_STATUS_SUCCESS)
            return std::unexpected(fail(error_code::compute, "SkinVAE query graph execution failed"));
