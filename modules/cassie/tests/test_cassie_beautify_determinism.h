...
/**************************************************************************/
/* Determinism golden + round-trip tests for the multiplayer beautify path.*/
/* xr-grid syncs raw stroke samples per peer and each peer reruns Beautify */
/* independently — this suite enforces that:                               */
/*  1. Beautify is bitwise reproducible across two calls in one process    */
/*     when CassieSample::creation_time is derived from sample_index * dt. */
/*  2. CassieStrokePacket encode/decode round-trips byte-for-byte.         */
/*  3. CassieSketchGraph::find_cycles emits the same canonical order       */
/*     regardless of stroke-insertion order.                               */
/* Cross-platform identity (Linux ↔ Windows ↔ macOS x64) is verified by   */
/* running the suite on every CI runner and comparing the serialized      */
/* control-point hash. The corpus_golden case stores the expected hex      */
/* string from the developer's primary platform; CI diff fails the test.  */
