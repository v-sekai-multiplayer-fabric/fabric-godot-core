...
RID _build_uniform_set(RenderingDevice *rd, const MasGpuHandle &h,
	...
	struct Slot { uint32_t binding; RID rid; bool is_uniform_buffer; };
	const Slot slots[] = {
...
CassieMasGpu::CassieMasGpu(RenderingDevice *p_external_rd) {
	...
	if (!_load_mas_pipelines(rd, shader, pipeline)) {
		...
		for (int i = 0; i < KERNEL_COUNT; ++i) {
			if (pipeline[i].is_valid()) { rd->free_rid(pipeline[i]); pipeline[i] = RID(); }
			if (shader[i].is_valid())   { rd->free_rid(shader[i]);   shader[i] = RID(); }
		}
		rd = nullptr;
	}
}
...
MasGpuHandle CassieMasGpu::build_mas_state(
	...
	if (box.size.x == 0) box.size.x = real_t(1e-6);
	if (box.size.y == 0) box.size.y = real_t(1e-6);
	if (box.size.z == 0) box.size.z = real_t(1e-6);

	struct MortonSlot { uint64_t code; int vert; };
	...
	int total_coarse = 0;
	for (int l = 1; l < num_levels; ++l) total_coarse += level_sizes[l];
	...
	for (int l = 1; l < num_levels; ++l) {
		...
		for (int s = 0; s < N_l; ++s) level_starts[s + 1] = level_starts[s] + counts[s];
		...
		cursors.resize(N_l);
		for (int s = 0; s < N_l; ++s) cursors[s] = level_starts[s];
		for (int i = 0; i < ni; ++i) {
			const int32_t s = map_l[i];
			if (s >= 0 && s < N_l) {
	...
	for (int l = 0; l < num_levels; ++l) per_level_total += level_sizes[l];
	...
	for (int i = 0; i <= ni; ++i) row_ptr_i[i] = int32_t(p_L_II.row_ptr[i]);
	...
	for (int l = 0; l < num_levels; ++l) ls_u32[l] = uint32_t(level_sizes[l]);
...
void CassieMasGpu::destroy_mas_state(MasGpuHandle &r_handle) {
	...
	for (RID &s : r_handle.uniform_sets_per_level_solve) {
		if (s.is_valid()) { rd->free_rid(s); s = RID(); }
	}
	...
	for (RID &b : r_handle.b_params_per_level) {
		if (b.is_valid()) { rd->free_rid(b); b = RID(); }
	}
	...
	RID *bufs[] = {
		&r_handle.b_params, &r_handle.b_row_ptr, &r_handle.b_col_idx,
		&r_handle.b_values, &r_handle.b_morton, &r_handle.b_sorted_idx,
		&r_handle.b_map_per_level, &r_handle.b_domain_offsets,
		&r_handle.b_m_inv_packed, &r_handle.b_r_per_level,
		&r_handle.b_z_per_level, &r_handle.b_r_input, &r_handle.b_z_output,
		&r_handle.b_coarse_offsets, &r_handle.b_coarse_indices,
		&r_handle.b_level_sizes, &r_handle.b_positions,
		&r_handle.b_connect_mask, &r_handle.b_dense_workspace,
	};
	for (RID *r : bufs) {
		if (r->is_valid()) {
			rd->free_rid(*r);
